...
import { ArrowUpRight, Building2, Check, Columns3, Minus } from 'lucide-react';
import './style.css';
          ...
          {[0, 1, 2, 3].map((groupIndex) => (
            <div className="brand-row__group" aria-hidden={groupIndex > 0 ? 'true' : undefined} key={groupIndex}>
              {trustedBrands.map((brand) => (
                <span className="brand-row__item" key={`${groupIndex}-${brand.name}`} role={groupIndex === 0 ? 'img' : undefined} aria-label={groupIndex === 0 ? brand.name : undefined} style={{ '--logo': `url(${brandLogos[brand.name]})` }} />
              ))}
            </div>
          ))}
        </div>
...
export function BlueprintSection() {
  ...
  // Stack depth: as each card sticks, the cards beneath it scale down and dim. Native scroll is
  // driven by Lenis, so a throttled scroll listener is enough.
  useEffect(() => {
    const stack = stackRef.current;
    if (!stack) return undefined;
    const cards = Array.from(stack.querySelectorAll('.blueprint-card'));
    let frame = 0;
    const update = () => {
      frame = 0;
      let current = 0;
      cards.forEach((card, i) => {
        ...
        if (card.getBoundingClientRect().top <= stickTop + 2) current = i;
      });
      cards.forEach((card, i) => {
        const depth = Math.max(0, current - i);
        card.style.setProperty('--depth', depth);
        card.classList.toggle('is-covered', depth > 0);
      });
      setActive(current);
    };
  ...
  const hasNumbers = (study) => !/\[/.test(study.followers) && !/\[/.test(study.reach);
  ...
  return (
    <section className="blueprint" id="blueprint" aria-labelledby="blueprint-title">
      ...
      <div className="blueprint__stack shell" ref={stackRef}>
        <ol className="blueprint__rail" aria-label={`Case study ${active + 1} of ${caseStudies.length}`}>
          {caseStudies.map((study, i) => <li key={study.slug} className={i === active ? 'is-active' : i < active ? 'is-done' : ''}><span>{String(i + 1).padStart(2, '0')}</span></li>)}
        </ol>
        {caseStudies.map((brand, index) => (
          <Link className="blueprint-card" to={`/case-study/${brand.slug}`} key={brand.slug} style={{ '--card-index': index }} aria-label={`${brand.name}: read the case study`}>
            <div className="blueprint-card__image liquid-glass-strong">
              <BrandMark brand={brand} />
              <span className="blueprint-card__handle">{brand.handle}</span>
            </div>
            <div className="blueprint-card__info">
              ...
              {hasNumbers(brand)
                ? <div className="blueprint-card__metrics"><span><b>Followers</b>{brand.followers}</span><span><b>Reach</b>{brand.reach}</span><span><b>Growth</b>{brand.growth}</span></div>
                : <div className="blueprint-card__chips" aria-label="What we built">{brand.built.slice(0, 4).map((item) => <span key={item}>{item}</span>)}</div>}
              <div className="blueprint-card__footer"><span className="blueprint-card__cta">Explore the full case study <ArrowIcon /></span></div>
            </div>
          </Link>
        ))}
      </div>
...
export function HowWorksSection() {
  ...
  const pickStep = (index) => { setUserPicked(true); setActiveStep((index + processSteps.length) % processSteps.length); };
  const phone = useMediaQuery('(max-width: 900px)');

  // Phones: swipe the tile left/right to move between steps (the step list is hidden there).
  const swipe = useRef({ x: 0, y: 0, active: false });
  const onPointerDown = (e) => { swipe.current = { x: e.clientX, y: e.clientY, active: true }; };
  const onPointerUp = (e) => {
    if (!swipe.current.active) return;
    swipe.current.active = false;
    const dx = e.clientX - swipe.current.x, dy = e.clientY - swipe.current.y;
    if (Math.abs(dx) < 40 || Math.abs(dx) < Math.abs(dy)) return;
    pickStep(activeStep + (dx < 0 ? 1 : -1));
  };
      ...
      <Reveal className="process__layout shell">
        <div className="process__intro">
          ...
          <div className="process__visual liquid-glass-frost" ref={visualRef} data-autoplay={autoplay && !userPicked} style={{ '--step-ms': `${STEP_MS}ms` }} onPointerDown={phone ? onPointerDown : undefined} onPointerUp={phone ? onPointerUp : undefined} onPointerCancel={() => { swipe.current.active = false; }}>
            ...
            <div className="process__visual-caption" key={phone ? active.number : 'static'}>
  ...
  return (
    <section className="packages" id="packages" aria-labelledby="packages-title">
      ...
      <Reveal className="shell">
        <div className="packages__actions liquid-glass-frost">
          <p>Need a side-by-side comparison or a custom clinic package?</p>
          <div className="packages__actions-row">
            <Link className="button button--primary" to="/packages/compare"><Columns3 aria-hidden="true" strokeWidth={1.8} />Compare tiers</Link>
            <Button variant="ghost"><Building2 aria-hidden="true" strokeWidth={1.8} />Request a clinic proposal</Button>
          </div>
        </div>
      </Reveal>
    </section>
    ...
    <section className={`founder${visible ? ' founder--visible' : ''}`} id="founder" ref={sectionRef} aria-labelledby="founder-title">
      <div className="founder__layout shell">
        <div className="founder__portrait">
          <img src="/assets/dr-hasia.jpg" alt="Doctor Hasia, founder of EVAE" loading="lazy" />
          <div className="eyebrow eyebrow--light founder__eyebrow--phone" aria-hidden="true">From the founder</div>
        </div>
...
function WhatsAppIcon() {
  // Official WhatsApp glyph (single filled path).
...
const CLINIC_KEY = 'evae-clinic-prompt';

// Clinics & group practices: a centred modal that opens once the visitor has scrolled past the
// Packages section. Shows once per session; closes with the ×, Escape, the backdrop or "Not now".
export function ClinicPrompt() {
  const [open, setOpen] = useState(false);
  const [dismissed, setDismissed] = useState(() => { try { return sessionStorage.getItem(CLINIC_KEY) === '1'; } catch { return false; } });
  const dialogRef = useRef(null);
  const openerRef = useRef(null);

  const close = () => {
    setOpen(false); setDismissed(true);
    try { sessionStorage.setItem(CLINIC_KEY, '1'); } catch { /* private mode */ }
    openerRef.current?.focus?.();
  };

  useEffect(() => {
    if (dismissed || open) return undefined;
    let frame = 0;
    const check = () => {
      frame = 0;
      const packages = document.getElementById('packages');
      if (!packages) return;
      if (packages.getBoundingClientRect().bottom < window.innerHeight * .6) { openerRef.current = document.activeElement; setOpen(true); }
    };
  ...
  // Open: lock the page (Lenis pauses on body overflow), move focus in, keep Tab inside, Escape closes.
  useEffect(() => {
    if (!open) return undefined;
    const previous = document.body.style.overflow;
    document.body.style.overflow = 'hidden';
    const dialog = dialogRef.current;
    const focusables = () => Array.from(dialog?.querySelectorAll('a[href], button:not([disabled])') ?? []);
    focusables()[0]?.focus();
    const onKey = (e) => {
  ...
  return (
    <div className="modal" onMouseDown={(e) => { if (e.target === e.currentTarget) close(); }}>
      <div className="modal__card liquid-glass-frost" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="clinic-prompt-title" aria-describedby="clinic-prompt-desc">
        <button type="button" className="modal__close" aria-label="Close" onClick={close}>×</button>
        <p className="modal__kicker">Clinics and group practices</p>
        <h3 id="clinic-prompt-title">Multiple practitioners? We build bespoke clinic packages.</h3>
        <p id="clinic-prompt-desc">Shared strategy, per-doctor content and one point of contact, scoped to your practice. Tell us about the clinic and we will put a proposal together.</p>
        <div className="modal__actions">
          <a className="button button--cta" href="#contact" onClick={close}><span>Request a clinic proposal</span><span className="button__icon" aria-hidden="true"><ArrowUpRight strokeWidth={2.2} /></span></a>
          <button type="button" className="modal__later" onClick={close}>Not now</button>
        </div>
      </div>
    </div>
  );
